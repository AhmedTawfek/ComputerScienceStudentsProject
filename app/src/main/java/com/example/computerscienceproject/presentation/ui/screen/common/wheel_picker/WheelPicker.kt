package com.example.computerscienceproject.presentation.ui.screen.common.wheel_picker
...
@Composable
fun PrimaryWheelPicker(
    modifier: Modifier = Modifier,
    items : List<String>,
    defaultSelectedIndex : Int = 0,
    onItemSelected : (Int) -> Unit = {}) {

    val state = rememberFWheelPickerState(initialIndex = defaultSelectedIndex)

    LaunchedEffect(state) {
        snapshotFlow { state.currentIndex }
            .collect {
                Log.i("WheelPicker", "currentIndex ${state.currentIndex}")
                onItemSelected(it)
            }
    }
    ...
    FVerticalWheelPicker(
        ...
        focus = {
            // Custom divider.
            FWheelPickerFocusVertical(dividerColor = MaterialTheme.colorScheme.primary, dividerSize = 1.dp)
        }) {
        Text(text = items[it], style = MaterialTheme.typography.titleSmall.copy(color = MaterialTheme.colorScheme.onBackground))
    }
}
...
@Preview
@Composable
private fun PrimaryWheelPickerPreview() {

    ComputerScienceProjectTheme {
    PrimaryWheelPicker(items = listOf("1","2","3","4","5","6","7"))
    }
